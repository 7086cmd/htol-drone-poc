...
impl<'a> Motion<'a> {
    ...
    pub async fn init(&mut self) -> Result<(), super::SensorError> {
        ...
        self.i2c
            ...
            .await
            .map_err(|err| super::SensorError::I2cError(err))?;
    ...
    pub async fn identity_check(&mut self) -> Result<bool, super::SensorError> {
        ...
        if buffer[0] == Self::EXPECTED_WHO_AM_I {
            ...
            Err(super::SensorError::IdentityMismatch)
        }
    }

    pub async fn read_acceleration(&mut self) -> Result<(f32, f32, f32), super::SensorError> {
    ...
    pub async fn read_gyroscope(&mut self) -> Result<(f32, f32, f32), super::SensorError> {
        let mut buffer = [0u8; 6];
    ...
    pub async fn read_temperature_mpu(&mut self) -> Result<i16, super::SensorError> {
    ...
    async fn read_raw_temperature(&mut self) -> Result<i32, super::SensorError> {
    ...
    async fn read_raw_pressure(&mut self) -> Result<i32, super::SensorError> {
    ...
    pub async fn read_barometer(&mut self) -> Result<(f32, f32), super::SensorError> {
        let ut = self.read_raw_temperature().await?;
        let up = self.read_raw_pressure().await?;

        let cal = match self.calib {
            Some(c) => c,
            None => return Err(super::SensorError::NoCalibrationData), // Calibration data not available
        };
    ...
    pub async fn read_magnetometer(&mut self) -> Result<(f32, f32, f32), super::SensorError> {
        let mut buffer = [0u8; 6];
